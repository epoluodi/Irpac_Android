...
public class PreviewPhotoViewPager extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        File file = new File(CameraCore.SAVEPPHOTOPATH);
        ...
        pagerAdapter = new PagerAdapter() {
            ...
            public Object instantiateItem(ViewGroup container, int position) {
                ...
                try {
                    ...
                    if (model == 1) {
                        bitmap = ImageCore.decodeBitmap(filephotos[position].getAbsolutePath(), 4);//获得缩小比例图片
//                        bitmapDrawable = new BitmapDrawable(bitmap);
                    }
                    if (model == 2) {
                        bitmap = ImageCore.decodeBitmap(filepath[position], 0);//获得缩小比例图片
//                        bitmapDrawable = new BitmapDrawable(bitmap);
                    }
...
//
                    if (model == 3) {
                        ...
                        if (map.containsKey("cachefile")) {
                            ...
                            bitmap = ImageCore.decodeBitmap(map.get("cachefile").toString(), 0);
//                            bitmapDrawable = new BitmapDrawable(bitmap);
...
//                    files[i] = filephotos[selectimages.get(i)].getAbsolutePath();
                    files[i] = ImageCore.copyCacheFile(filephotos[selectimages.get(i)].getAbsolutePath());
                    Bitmap bitmap = ImageCore.bitbmpfrommediaLocal(files[i], 0);
//                    Bitmap bitmap = ImageCore.decodeBitmap1024(files[i]);
                    ByteArrayOutputStream baos = new ByteArrayOutputStream();
                    bitmap.compress(Bitmap.CompressFormat.JPEG, 100, baos);
                    try {
                        FileOutputStream fileOutputStream = new FileOutputStream(Init.getContext().getCacheDir()
                                + "/" + files[i] + "_200.jpg");
...
//        layoutParams1.addRule(RelativeLayout.CENTER_HORIZONTAL);
        ...
        RelativeLayout.LayoutParams layoutParams2 = new RelativeLayout.LayoutParams(Common.dip2px(50),
                Common.dip2px(50));
            ...
            try {
                for (int i = 0; i < urlfiles.length; i++) {
                    ...
                    File cachefile = new File(getCacheDir() + "/"
                            + urlfiles[i] + ".jpg");
                    if (!cachefile.exists()) {
                        FileDownload fileDownload = new FileDownload(url, urlfiles[i]);
